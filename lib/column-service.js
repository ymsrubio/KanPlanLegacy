...
export function createColumn(db, accountId, { name, wip_limit = null }) {
...
export function updateColumn(db, accountId, columnId, { name, wip_limit }) {
...
export function deleteColumn(db, accountId, columnId) {
  const column = db.prepare('SELECT * FROM columns WHERE id = ? AND account_id = ?').get(columnId, accountId);
  if (!column) {
    throw new Error('Column not found');
  }
